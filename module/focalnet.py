...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=4, in_chans=3, embed_dim=96, use_conv_embed=False, norm_layer=None, is_stem=False):
        ...
        if isinstance(use_conv_embed,bool):
            if use_conv_embed:
        ...
        elif isinstance(use_conv_embed,Iterable):
            assert len(use_conv_embed)==3,'the shape of use_conv_embed should be [kernel_size,padding,stride]'
            kernel_size,padding,stride = use_conv_embed
            self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=kernel_size, stride=stride, padding=padding)
...
if __name__ == '__main__':
    ...
    model = FocalNet(depths=[2, 2, 6, 2], embed_dim=96, focal_levels=[3, 3, 3, 3],use_conv_embed=[2,0,2],num_classes=0)
    # print(model); 
    print(model(x).size())
